...
const { dateKey, addDays, startOfWeek } = require("../utils/records");
...
function rangeDefaults(){const start=startOfWeek();return{start,end:addDays(start,20)};}
function isoDay(value){const day=new Date(`${value}T12:00:00`).getDay();return day||7;}

async function ensureSchedule(userId,startValue,endValue){
    const defaults=rangeDefaults(),start=startValue||defaults.start,end=endValue||defaults.end;
    ...
    await withTransaction(async(client)=>{
        ...
        for(let current=start;current<=end;current=addDays(current,1)){
            for(const row of rows.filter((item)=>item.day_of_week===isoDay(current)&&current>=dateKey(item.starts_on))){
                const existing=await client.query("SELECT * FROM scheduled_workouts WHERE user_id=$1 AND template_id=$2 AND workout_date=$3 FOR UPDATE",[userId,row.template_id,current]);
                if(!existing.rowCount){
                    const id=crypto.randomUUID(),eventId=crypto.randomUUID();
                    await planner.insertEvent(client,userId,{title:row.workout_name,type:"gym",date:current,startTime:String(row.start_time).slice(0,5),endTime:String(row.end_time).slice(0,5),completed:false,notes:"",metadata:{}},eventId,{entityType:"workout",entityId:id,templateId:row.template_id,scheduleId:row.schedule_id,recurring:true});
                    await client.query(`INSERT INTO scheduled_workouts(id,user_id,event_id,template_id,schedule_id,source,name,workout_type,workout_date,start_time,end_time,planned_minutes,exercises)
                        VALUES($1,$2,$3,$4,$5,'recurring',$6,$7,$8,$9,$10,$11,$12)`,[id,userId,eventId,row.template_id,row.schedule_id,row.workout_name,row.workout_type,current,row.start_time,row.end_time,row.planned_minutes,JSON.stringify(row.exercises)]);
                }else if(existing.rows[0].status==="planned"&&current>=dateKey()){
                    const workout=existing.rows[0];
                    await client.query("UPDATE scheduled_workouts SET schedule_id=$3,name=$4,workout_type=$5,start_time=$6,end_time=$7,planned_minutes=$8,exercises=$9,updated_at=NOW() WHERE id=$1 AND user_id=$2",[workout.id,userId,row.schedule_id,row.workout_name,row.workout_type,row.start_time,row.end_time,row.planned_minutes,JSON.stringify(row.exercises)]);
                    await client.query("UPDATE calendar_events SET title=$3,start_time=$4,end_time=$5,duration_minutes=$6,metadata=$7,updated_at=NOW() WHERE id=$1 AND user_id=$2",[workout.event_id,userId,row.workout_name,row.start_time,row.end_time,row.planned_minutes,{entityType:"workout",entityId:workout.id,templateId:row.template_id,scheduleId:row.schedule_id,recurring:true}]);
                }
            }
        }
    });
...
async function updateWorkout(userId,id,input){return withTransaction(async(client)=>{const current=await client.query("SELECT * FROM scheduled_workouts WHERE id=$1 AND user_id=$2 FOR UPDATE",[id,userId]);if(!current.rowCount)fail("Workout not found",404);if(Boolean(current.rows[0].completed)!==Boolean(input.completed))fail(current.rows[0].completed?"Reopen completed workouts through the reopen action":"Complete workouts through the workout log so actual time is recorded",409);const planned=schedule(input,"Workout");const result=await client.query("UPDATE scheduled_workouts SET name=$3,workout_type=$4,workout_date=$5,start_time=$6,end_time=$7,planned_minutes=$8,notes=$9,updated_at=NOW() WHERE id=$1 AND user_id=$2 RETURNING *",[id,userId,input.name,input.workoutType,planned.date,planned.startTime,planned.endTime,planned.plannedMinutes,input.notes]);await client.query("UPDATE calendar_events SET title=$3,event_date=$4,start_time=$5,end_time=$6,duration_minutes=$7,notes=$8,metadata=$9,updated_at=NOW() WHERE id=$1 AND user_id=$2",[current.rows[0].event_id,userId,input.name,planned.date,planned.startTime,planned.endTime,planned.plannedMinutes,input.notes,{entityType:"workout",entityId:id,workoutType:input.workoutType}]);return map.workout(result.rows[0]);});}
async function deleteWorkout(userId,id){return withTransaction(async(client)=>{const current=await client.query("SELECT * FROM scheduled_workouts WHERE id=$1 AND user_id=$2 FOR UPDATE",[id,userId]);if(!current.rowCount)fail("Workout not found",404);if(current.rows[0].source==="recurring"){await client.query("UPDATE scheduled_workouts SET status='cancelled',completed=FALSE,actual_minutes=0,completed_at=NULL,updated_at=NOW() WHERE id=$1 AND user_id=$2",[id,userId]);await client.query("DELETE FROM workout_logs WHERE scheduled_workout_id=$1 AND user_id=$2",[id,userId]);await client.query("DELETE FROM activity_sessions WHERE workout_id=$1 AND user_id=$2",[id,userId]);await client.query("DELETE FROM calendar_events WHERE id=$1 AND user_id=$2",[current.rows[0].event_id,userId]);}else{await client.query("DELETE FROM scheduled_workouts WHERE id=$1 AND user_id=$2",[id,userId]);await client.query("DELETE FROM calendar_events WHERE id=$1 AND user_id=$2",[current.rows[0].event_id,userId]);}});}
...
async function analytics(userId,start,end){await ensureSchedule(userId,start,end);const rows=(await getPool().query("SELECT * FROM scheduled_workouts WHERE user_id=$1 AND workout_date BETWEEN $2 AND $3 AND status<>'cancelled'",[userId,start,end])).rows;const completed=rows.filter((row)=>row.completed);return{range:{planned:rows.length,completed:completed.length,missed:rows.filter((row)=>!row.completed&&String(row.workout_date)<dateKey()).length,completionRate:rows.length?Math.round(completed.length/rows.length*1000)/10:0,plannedMinutes:rows.reduce((sum,row)=>sum+row.planned_minutes,0),actualMinutes:completed.reduce((sum,row)=>sum+row.actual_minutes,0),totalWorkoutTimeMinutes:completed.reduce((sum,row)=>sum+row.actual_minutes,0)}};}

module.exports={getTemplates,createTemplate,updateTemplate,deleteTemplate,ensureSchedule,listWorkouts,listLogs,createWorkout,updateWorkout,deleteWorkout,completeWorkout,reopenWorkout,analytics};
