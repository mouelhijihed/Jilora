...
async function analytics(userId,startValue,endValue){
    const timeZone=await userTimeZone(userId,getPool()),today=dateKeyInTimeZone(timeZone),start=startValue||startOfWeek(today),end=endValue||today;
    const [prefs,studySessions,activity,workouts,workSessions,homework,subjects]=await Promise.all([
        ...
        getPool().query("SELECT * FROM activity_sessions WHERE user_id=$1 AND status='completed' AND (completed_at AT TIME ZONE $4)::date BETWEEN $2 AND $3",[userId,start,end,timeZone]),
    ...
    const timestampDate=(value)=>dateKeyInTimeZone(timeZone,value instanceof Date?value:new Date(value));
