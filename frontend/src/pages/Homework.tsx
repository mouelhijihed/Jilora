...
export function Homework() {
    ...
    const completed = homeworkTasks.filter((task) => task.status === "completed");
    ...
    return <main className="homework-page page-shell"><header className="tracking-header"><div><p className="eyebrow">Assignments</p><h1>Homework</h1><p>Keep due dates, priorities, and completion status in one queue.</p></div><button className="primary-button" type="button" onClick={() => openEditor()}>Add homework</button></header>{error && <div className="notice notice-error">{error}</div>}<section className="metric-grid"><div className="metric-card"><span>Due today</span><strong>{todayTasks.length}</strong></div><div className="metric-card"><span>Upcoming</span><strong>{upcoming.length}</strong></div><div className="metric-card"><span>Overdue</span><strong>{overdue.length}</strong></div><div className="metric-card"><span>Completed</span><strong>{completed.length}</strong></div></section><section className="tracking-grid"><article className="tracking-card"><h2>Today's tasks</h2><div className="tracking-list">{todayTasks.length ? todayTasks.map(renderTask) : <p className="empty-state">Nothing due today.</p>}</div><h2 className="tracking-subsection-title">Upcoming</h2><div className="tracking-list">{upcoming.length ? upcoming.map(renderTask) : <p className="empty-state">No upcoming homework.</p>}</div></article><article className="tracking-card"><h2>Needs attention</h2><div className="tracking-list">{overdue.length ? overdue.map(renderTask) : <p className="empty-state">No overdue tasks. Keep it that way.</p>}</div><h2 className="tracking-subsection-title">Completed</h2><div className="tracking-list">{completed.slice(-6).reverse().map(renderTask)}</div></article></section>{editorOpen && <HomeworkTaskEditor task={editing} onClose={() => setEditorOpen(false)} onSave={save} onDelete={editing ? () => deleteHomeworkTask(editing.id) : undefined} />}</main>;
}
