        ...
        {!state?.partnership ? <section className="partner-connect" aria-labelledby="connect-heading">
            ...
            <section className="partner-main-grid">
                <div className="partner-column">
                    ...
                    <section className="partner-section" aria-labelledby="goals-heading">
                        ...
                        <div className="goal-list">{goals.map((goal) => <article className="goal-row" key={goal.id}>
                            ...
                            <div className="progress-track"><span style={{ width: `${goal.percent}%` }} /></div><div className="goal-progress"><span>{goalValue(Math.max(0, goal.progress), goal.type)} / {goalValue(goal.target, goal.type)}</span><strong>{goal.status === "completed" ? "Completed" : goal.status === "missed" ? "Missed" : `${goal.percent}%`}</strong></div>{goal.status === "active" && <button className="small-button" type="button" onClick={() => void run(`goal-complete-${goal.id}`, () => partnerService.completeGoal(goal.id), "Goal marked completed.")} disabled={busy === `goal-complete-${goal.id}`}><FiCheck aria-hidden="true" />Mark completed</button>}
                ...
                <aside className="partner-column">
                    ...
                    <section className="partner-section"><div className="section-header"><div><p className="eyebrow">Recent activity</p><h2>Shared feed</h2></div></div><div className="activity-list">{(showAllActivity ? shared?.activity : shared?.activity.slice(0, 5))?.map((activity) => <div className="activity-row" key={activity.id}><span className="activity-mark" aria-hidden="true"><FiCheck /></span><div><strong>{activity.actor.firstName} {activity.message.charAt(0).toLowerCase() + activity.message.slice(1)}</strong><time>{timeAgo(activity.createdAt)}</time></div></div>)}{!shared?.activity.length && <p className="empty-state">Shared activities will appear here only when the person has enabled the relevant sharing category.</p>}{shared && shared.activity.length > 5 && <button className="small-button" type="button" onClick={() => setShowAllActivity((current) => !current)}>{showAllActivity ? "Show less" : "Show more"}</button>}</div></section>

                    {shared?.partner.tasks && <section className="partner-section"><div className="section-header"><div><p className="eyebrow">Shared tasks</p><h2>Open tasks</h2></div></div><div className="activity-list">{(showAllTasks ? shared.partner.tasks : shared.partner.tasks.slice(0, 5)).map((task) => <div className="activity-row" key={`${task.category}-${task.title}-${task.dueDate || "none"}`}><span className="activity-mark" aria-hidden="true"><FiCheck /></span><div><strong>{task.title}</strong><time>{task.category}{task.dueDate ? ` / due ${task.dueDate}` : ""}</time></div></div>)}{!shared.partner.tasks.length && <p className="empty-state">No shared tasks.</p>}{shared.partner.tasks.length > 5 && <button className="small-button" type="button" onClick={() => setShowAllTasks((current) => !current)}>{showAllTasks ? "Show less" : "Show more"}</button>}</div></section>}

                    {shared?.partner.workouts && <section className="partner-section"><div className="section-header"><div><p className="eyebrow">Shared workouts</p><h2>Upcoming gym tasks</h2></div></div><div className="activity-list">{(showAllWorkouts ? shared.partner.workouts : shared.partner.workouts.slice(0, 5)).map((workout) => <div className="activity-row" key={`${workout.name}-${workout.workoutDate}`}><span className="activity-mark" aria-hidden="true"><FiCheck /></span><div><strong>{workout.name}</strong><time>{workout.workoutDate} / {workout.completed ? "Completed" : "Planned"}</time></div></div>)}{!shared.partner.workouts.length && <p className="empty-state">No shared workouts.</p>}{shared.partner.workouts.length > 5 && <button className="small-button" type="button" onClick={() => setShowAllWorkouts((current) => !current)}>{showAllWorkouts ? "Show less" : "Show more"}</button>}</div></section>}
